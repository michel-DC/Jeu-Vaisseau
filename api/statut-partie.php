...
if ($result && $partie = mysqli_fetch_assoc($result)) {
    ...
    // Ensure drone arrays exist for older games that may not have been initialized
    ...
    if (empty($partie['joueur1_drones'])) {
        $partie['joueur1_drones'] = $default_drones;
    }
    if (empty($partie['joueur2_drones'])) {
        $partie['joueur2_drones'] = $default_drones;
    }
